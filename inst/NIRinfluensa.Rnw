...
InfluData <- intensivberedskap::NIRsqlPreInfluensa(datoFra = '2018-01-01', datoTil = Sys.Date(), preprosess=1)
...
# varObs <- c("Bekr" ,   "DeadPatientDuring24Hours", "DischargedIntensiveStatus", "Influensa",
...
InfluData <- InfluData[which(InfluData$Sesong %in% sesongNaa), ]
...
#
# egetRHF <- as.character(InfluData$RHF[match(reshID, InfluData$ReshId)])
# egetHF <- InfluData$HF[match(reshID, InfluData$ReshId)]
#
...
Influensarapportering fra NIR, sesongen \Sexpr{sesongNaa} }
...
InfluData$RHF <- as.character(InfluData$RHF)
    ...
    print(xtable::xtable(RegHF, digits=0,
          caption = 'Antall inneliggende intensivpasienter i hver enhet'),
...
print(xtable::xtable(TabUkeRHF[indAntRader,], digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
		caption=paste0('Influensatilfeller per uke og region sesongen', sesongNaa,' (B - bekreftet, M - mistenkt)',
...
sesongNaa,'. (B - bekreftet, M - mistenkt)', ' Uker med 0 tilfeller vises ikke.')),
		add.to.row = add.to.row,
		sanitize.rownames.function = identity)
@
...
print(xtable::xtable(TabUkeTot[indAntRader,], digits=0, #align=c('l', rep('r', ncol(TabUkeTot))),
		caption=paste0('Antall influensatilfeller sesongen', sesongNaa, '.', ' Uker med 0 tilfeller vises ikke.')),
...
print(xtable::xtable(TabUkeTotFerdig[indAntRader,], digits=0, #align=c('l', rep('r', ncol(TabUkeTot))),
		caption=paste0('Antall influensatilfeller basert på ferdigstilte skjema, sesongen', sesongNaa,'.',
		               ' Uker med 0 tilfeller vises ikke.')),
		sanitize.rownames.function = identity)
@
\end{frame}
...
\begin{frame}[fragile] {Antall registrerte pasienter på hvert sykehus}
<<'OpphPrHF', results='asis', warnings=FALSE>>=
    #InfluDataDum <- if (enhetsNivaa == 'HF') {InfluData} else {InfluDataAlle}
    #if (enhetsNivaa == 'HF') {
      # RegHF <- InfluData %>%
...
print(
  xtable::xtable(RegHF , digits=0,
                 caption='Totalt antall registrerte intensivpasienter fra sesongstart til rapporteringstidspunktet.')
      ,include.rownames = FALSE)
@
\end{frame}
...
colnames(TabRisiko) <- paste0(paste0("\\multicolumn{1}{p{", bredde, "cm}}{"), colnames(TabRisiko),'}')
  print(
    xtable::xtable(TabRisiko,
                   ...
                   caption=paste0('Hyppighet av risikofaktorer blant intensivpasienter fra sesongstart
...
xtable::xtable(FerdigBekr$Tab,
               digits=0,
               align = c('l','r','r','c', 'r','r'),
               caption = paste0('Verdier basert på intensivpasienter fra sesongstart til rapportdato.
