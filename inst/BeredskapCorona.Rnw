...
\item Rapporten er basert på alle registreringer gjort inntil 3 timer før rapporten ble lastet ned.
\end{itemize}
...
tidsenhet <- 'dag'
  ...
  RegData$EnhetsNivaaVar <- as.factor(RegData[ ,enhetsNivaa])
...
navnEnh <- levels(RegData$EnhetsNivaaVar) #unique(RegData$EnhetsNivaaVar)
...
% <<'Dod', results='asis'>>=
% indDod <- which(RegData$DischargedIntensiveStatus==1)
% TabRHF <- table(RegData$Dag[indDod], RegData$RHF[indDod])
% if (length(indDod)==0) {
...
AlderBekr <- round(summary(RegData$Alder[RegData$Korona=='B']))

tabAlder <- TabAlder(RegData, valgtRHF='Alle')
@

De registrerte er mellom \Sexpr{Alder[1]} og \Sexpr{Alder[6]} år med en gjennomsnittsalder på \Sexpr{Alder[4]}.
For de bekreftede tilfellene er \Sexpr{AlderBekr[1]} og \Sexpr{AlderBekr[6]} år
med en gjennomsnittsalder på \Sexpr{AlderBekr[4]}.
...
xtable::xtable(TabAlder(RegData=RegData[which(RegData$Korona=='B'), ]
                        , valgtRHF='Alle'),
               digits = 0,
...
<<'Risikofaktorer', results='asis'>>=
    TabRisiko <- RisikofaktorerTab(RegData=RegData[which(RegData$Korona=='B'), ])
