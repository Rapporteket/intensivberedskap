...
# Registreringer i limbo
    ...
    RegILimboTxt <- if (AntLimbo>0) {paste0('\\item Det er ', AntLimbo, ' skjema i ', RHFtxt, ' som mangler ferdigstillelse. Dvs. opphold registrert som utskrevet, uten at skjema er ferdigstilt.')} else {''}
...
print(xtable::xtable(TabTidEnh[(max(1,AntRader-20)):AntRader, ], digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
		caption=paste0('Coronatilfeller siste 20 dager. Totalt er sum fra 10.mars. \n (B - bekreftet, M - mistenkt)')),
  ...
  AntRader <- dim(TabTidEnh)[1]
  xtable::xtable(TabTidEnh[max(1,AntRader-10):AntRader, ], digits=0,
		caption='Coronatilfeller siste 20 dager i egen region, samt hele landet. Totalt er sum fra 10.mars.
		\\\ (B - bekreftet, M - mistenkt)')
}
...
xtable::xtable(FerdigBekr$Tab,
               digits=0,
               align = c('l','r','r','c', 'r','r')
                , caption=paste0('Ferdigstilte opphold, bekreftede tilfeller (',  FerdigBekr$Ntest,' skjema).
...
\begin{frame}[fragile] {Corona, aldersfordeling, alle}
...
\begin{frame}[fragile] {Corona, aldersfordeling, bekreftede}
\Sexpr{txtBekr}
