...
# test <- RegDataAlleOpph %>%
#          dplyr::group_by(RHF, HF, Sykehus) %>%
...
Rapporten baserer seg på alle registrerte tilfeller, hvorav \Sexpr{antUbekr} ikke har påvist smitte med SARS-CoV-2. I dokumentet skilles det ikke på om skjema er ferdigstilt eller ikke, dersom ikke annet er oppgitt. At et opphold er ferdigstilt vil si at opplysningene er kontrollert og ferdig registrert.
...
EnhetsNivaaVarOpph <- switch(enhetsNivaa,
                         Alle = 'RHF',
                         RHF = 'HF',
                         HF = 'Sykehus')
...
TabTidEnh <- ftable(RegDataAlleOpph[ , c(TidsVar, 'RHF')], #, 'Korona'
                    row.vars =TidsVar)
navnEnh <- unique(RegDataAlleOpph$EnhetsNivaaVarOpph)
...
# 		sanitize.rownames.function = identity)
  ...
  RegDataOpph$EnhetsNivaaVarOpph <- as.factor(RegDataOpph[ ,EnhetsNivaaVarOpph]) #RegDataOpph = RegDataOpphAlle for enhetsnivå 'Alle'
  TabEnh <- table(RegDataOpph[, c('TidsVar', 'EnhetsNivaaVarOpph')])
      # table(RegDataOpph[RegDataOpph$EnhetsNivaaOpph == enhet,
      #                           c('TidsVar', 'EnhetsNivaaVarOpph')])
...
\begin{frame}[fragile] {Registrerte pasienter i hvert \Sexpr{visningsNivaaLav}}
