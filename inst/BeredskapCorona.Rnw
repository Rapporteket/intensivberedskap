...
if (enhetsNivaa=='Alle'){
TabTidEnh <- ftable(RegDataAlle[ , c('Dag', 'RHF')], #, 'Korona'
                    row.vars = 'Dag')
...
for hele tidsperioden og siste tre måneder. Søylene maskeres hvis det er færre enn tre kvinner eller menn.}
\end{figure}
...
%  % [fragile] {UTGÅR - Antall registrerte opphold per dag}
%
% <<'TabTotOpph', results='asis'>>=
%     #Vise bare RHF og landet for RHF-nivå som før..? Pga. veldig lange HF-navn
% EnhetsNivaaVar <- switch(enhetsNivaa,
%                          Alle = 'RHF',
%                          RHF = 'HF',
...
%
% if (enhetsNivaa=='Alle'){
% TabTidEnh <- ftable(RegDataAlleOpph[ , c(Dag, 'RHF')], #, 'Korona'
%                     row.vars =Dag)
% navnEnh <- unique(RegDataAlleOpph$EnhetsNivaaVar)
% TabTidEnh <- as.matrix(TabTidEnh)
%
% TabTidEnh <- addmargins(TabTidEnh, FUN=list(Totalt=sum, 'Hele landet' = sum), quiet=TRUE)
% # add.to.row <- list(pos = list(-1), command = NULL)
...
%   TabEnh <- table(RegDataOpph[, c('Dag', 'EnhetsNivaaVarOpph')])
%
%   antEnh <- dim(TabEnh)[2]
%     TabEnh <- addmargins(TabEnh, margin=2, sum)
%   colnames(TabEnh) <- c(colnames(TabEnh)[1:antEnh], paste0('eget ', enhetsNivaa)) #enhet)
...
%   TabTidEnh <- cbind(
%                   TabEnh,
%                   'Hele landet' = table(RegDataAlleOpph$Dag))
%
%   TabTidEnh <- addmargins(TabTidEnh, margin=1, FUN= list(Totalt=sum), quiet=TRUE)
%   if(enhetsNivaa == 'RHF') {TabTidEnh <- TabTidEnh[,-antEnh:-1]}
%
%   AntRader <- dim(TabTidEnh)[1]
%   xtable::xtable(TabTidEnh[max(1,AntRader-20):AntRader, ], digits=0,
% 		caption='Antall nye opphold siste 20 dager, samt totalt antall fra pandemistart til rapporteringstidspuktet.')
%   }
% @
% \end{frame}
%
