...
#Inngangsparametre: (tidl. valgtRHF)
...
if (enhetsNivaa != 'Alle'){
  RegData <- subset(RegData, RegData[ , enhetsNivaa] == RegData[match(reshID, RegData$ReshId), enhetsNivaa])
  RegDataOpph <- subset(RegDataOpph, RegDataOpph[ , enhetsNivaa] == RegDataOpph[match(reshID, RegDataOpph$ReshId), enhetsNivaa])
  }
visningsNivaaLav <- ifelse(enhetsNivaa=='HF', 'sykehus', 'HF')
...
Rapporten er kjørt for Enhetsnivå:  \Sexpr{ifelse(enhetsNivaa == 'Alle', 'Hele landet', enhetsNivaa)}, Enhet: \Sexpr{enhet}
...
Antall inneliggende pasienter per helseforetak på rapporteringstidspunktet. (Definisjon på inneliggende: Pasienter hvor ut-tid fra intensiv ikke er registrert.)
...
RegHF <- if (enhetsNivaa == 'HF') {
  RegDataDum[inneligg,] %>%
  dplyr::group_by(ShNavnUt) %>%
...
EnhetsNivaaVar <- switch(enhetsNivaa,
                         ...
                         RHF = 'HF',
                         HF = 'ShNavn')
  RegDataAlleOpph$EnhetsNivaaVar <- as.factor(RegDataAlleOpph[ ,EnhetsNivaaVar])
...
# 		sanitize.rownames.function = identity)
  ...
  TabTidEnh <- cbind(
    table(RegDataAlleOpph[RegDataAlleOpph$EnhetsNivaaVar==enhet, 'TidsVar']), #c('TidsVar', 'Korona')
    'Hele landet' = table(RegDataAlleOpph$TidsVar))
  colnames(TabTidEnh) <- c(enhet, 'Hele landet')
...
\begin{frame}[fragile] {Registrerte opphold i hvert helseforetak}
...
Totalt <- c('', 'Totalt', sum(RegHF$`Antall pasienter`))
#RegHF <- rbind(as.data.frame(RegHF),                Totalt)
print(xtable::xtable(rbind(as.data.frame(RegHF), Totalt), row.names=FALSE, digits=0,
...
TabAlder <- TabAlder(RegData=RegDataAlle, valgtRHF=valgtRHF)$Tab
...
dum <- FigFordelingKjonnsdelt(RegData = RegData, valgtVar = 'Alder',
                              #valgtRHF= valgtRHF, bekr = 1,
                              outfile = 'AlderKjonn.pdf')
@

\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{AlderKjonn.pdf}
\caption{Fordeling av kjønn i ulike aldersgrupper på rapporteringstidspunktet.}
\end{figure}
...
if (dim(RegData)[1]>2) {
  txtRisiko <- ''
    TabRisiko <- RisikofaktorerTab(RegData=RegData)$Tab
  ...
  txtRisiko <- paste0('Det er registrert færre enn 3 tilfeller i region ',
                      valgtRHF, '.')
}
...
\begin{frame}[fragile] {Resultater for ferdigstilte opphold}
...
FerdigBekr <- oppsumFerdigeRegTab(RegData=RegData, valgtRHF=valgtRHF)
