...
# add.to.row$command <- command
    UtData <- TabTidEnhet(RegData=CoroData, tidsenhet='dag', enhetsNivaa='RHF') #, datoTil=Sys.Date(), reshID=0
print(xtable::xtable(UtData$TabTidEnh, digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
...
MechanicalRespirator=1 også hvis det bare er registrert tidspunkt for start av respirator.
...
# AntBruktResp <- sum(CoroData$MechanicalRespirator==1, na.rm=T)
# AntBruktECMO <- sum(CoroData$EcmoDurationInHours>0, na.rm=T)
#
# AntIrespNaa <- sum(!(is.na(CoroData$MechanicalRespiratorStart))) -
#   sum(!(is.na(CoroData$MechanicalRespiratorEnd)))
# AntIECMONaa <- sum(!(is.na(CoroData$EcmoStart))) - sum(!(is.na(CoroData$EcmoEnd)))
# AntPaaIntNaa <- N - sum(!(is.na(CoroData$DateDischargedIntensive)))
#
# TabHjelp <- rbind(
#   'På intensiv nå' = AntIECMONaa(c(1, 100/N)),
#   'På respirator nå' = AntIrespNaa(c(1, 100/AntPaaIntNaa)),
#   'På ECMO nå' = AntIresp(c(1, 100/AntPaaIntNaa)),
