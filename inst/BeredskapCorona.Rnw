...
Rapporten er kjørt for Rolle: (parameter som per nå ikke sendes inn i rapporten)   ; Enhetsnivå:  \Sexpr{ifelse(valgtRHF=='Alle', 'Hele landet', 'RHF')}  ; Enhet: \Sexpr{valgtRHF}
...
print(xtable::xtable(rbind(as.data.frame(RegHF), Totalt), row.names=FALSE, digits=0,
                     caption='Antall inneliggende intensivpasienter i hvert helseforetak'),
...
# 		sanitize.rownames.function = identity)
  ...
  TabTidEnh <- cbind(
    table(RegDataAlleOpph[RegDataAlleOpph$RHF==valgtRHF, 'TidsVar']), #c('TidsVar', 'Korona')
    'Hele landet' = table(RegDataAlleOpph$TidsVar))
  colnames(TabTidEnh) <- c(valgtRHF, 'Hele landet')
