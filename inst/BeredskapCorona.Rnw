...
for hele tidsperioden og siste tre måneder.}
\end{figure}
...
txtRisiko <- ''
    ...
    if (enhetsNivaa == 'HF') {
      ...
      TabRisiko <- RisikofaktorerTab(RegData=RegDataAlleUXm)$Tab
    }
...
RegDataAlle2m <- RegDataAlle[which(RegDataAlle$InnDato >= datoFraXmnd), ]
RegData2m <- RegData[which(RegData$InnDato >= datoFraXmnd), ]
...
\begin{frame}[fragile] {Resultater for ferdigstilte opphold, siste tre måneder}
