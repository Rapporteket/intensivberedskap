...
Antall inneliggende pasienter per \Sexpr{visningsNivaaLav}
\Sexpr{ifelse(enhetsNivaa == 'HF', enhet, '')}
...
if (dim(RegData)[1]>2) {
  txtRisiko <- ''
    if (enhetsNivaa == 'HF') {
      TabRisiko <- RisikofaktorerTab(RegData=RegDataAlle[RegDataAlle$RHF==egetRHF,])$Tab
      colnames(TabRisiko) <- paste0(c('Antall', 'Andel'), ', ', egetRHF)
    } else {
      TabRisiko <- RisikofaktorerTab(RegData=RegDataAlle)$Tab
    }

  if (enhetsNivaa %in% c('RHF', 'HF')) {
     TabRisikoEget <- RisikofaktorerTab(RegData=RegData)$Tab
    ...
    xtable::xtable(TabRisiko,
                   ...
                   caption='Hyppighet av risikofaktorer på rapporteringstidspunktet.
...
RegDataAlle2m <- RegDataAlle[which(RegDataAlle$InnDato >= as.Date(Sys.Date()-60)), ]
RegData2m <- RegData[which(RegData$InnDato >= as.Date(Sys.Date()-60)), ]
...
xtable::xtable(FerdigBekr$Tab,
               digits=0,
               align = c('l','r','r','c', 'r','r'),
               caption = paste0('Verdier på rapporteringstidspunktet.
...
#oppsumFerdigeRegTab(RegData=RegData, valgtRHF=valgtRHF)$Tab
FerdigBekr <- oppsumFerdigeRegTab(RegData=RegData, datoFra = as.Date(Sys.Date()-60))
xtable::xtable(FerdigBekr$Tab,
               digits=0,
               align = c('l','r','r','c', 'r','r'),
