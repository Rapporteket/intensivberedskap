...
if (dim(RegData)[1]>2) {
  txtRisiko <- ''
    if (enhetsNivaa == 'HF') {
      ...
      TabRisiko <- RisikofaktorerTab(RegData=RegDataAlleXm)$Tab
    }

  if (enhetsNivaa %in% c('RHF', 'HF')) {
     TabRisikoEget <- RisikofaktorerTab(RegData=RegDataXm)$Tab
     colnames(TabRisikoEget) <- paste0(c('Antall', 'Andel'), ', eget', enhetsNivaa)
     TabRisiko <- cbind(
       TabRisikoEget,
       TabRisiko
...
\begin{frame}[fragile] {Resultater for ferdigstilte opphold, \Sexpr{enhetTxt}}
