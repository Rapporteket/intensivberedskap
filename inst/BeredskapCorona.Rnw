...
xtable::xtable(oppsumFerdigeRegTab(RegData=CoroData, valgtRHF=valgtRHF)$Tab,
               digits=0,
               align = c('l','r','c','r', 'r'),
               caption='Bekreftede tilfeller, ferdigstilte registreringer.
               IQR (Inter quartile range) - 50% av oppholdene er i dette intervallet, mens det er 25% som er mindre og 25% som er større.')
@
\end{frame}
...
xtable::xtable(tabAlder$Tab,
               digits = 0,
               caption='Fordeling i aldersgrupper')

xtable::xtable(TabAlder(RegData=RegData[which(RegData$Bekreftet==1), ]
                        , valgtRHF='Alle')$Tab,
               digits = 0,
